...
API_SERVER_ENABLED = os.getenv("API_SERVER_ENABLED", "false").lower() == 'true'
...
MILVUS_NUM_PARTITIONS = int(os.getenv("MILVUS_NUM_PARTITIONS", "64"))
...
if NATS_ENABLED and (not NATS_URL or not NATS_USER or not NATS_PASS or not NATS_SUFFIX):
...
if API_SERVER_ENABLED and API_SERVER_PORT == STATUS_SERVER_PORT:
    Exception("When using API server make sure the status port and api server port are different!")
    exit(1)
